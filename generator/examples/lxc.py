...
def create(last_host_id, last_container_id, last_link_id, starting_address):
    ...
    routeflow1_component = generate.create_container(host1, "rfvm", ContainerType.LXC,
                                                           "ubuntu", BackingStore.LVM, rfvm_scripts)
    components[routeflow1_component.component_id] = routeflow1_component

    routeflow2_component = generate.create_container(host1, "rfvm", ContainerType.LXC,
                                                           "ubuntu", BackingStore.LVM, rfvm_scripts)
    components[routeflow2_component.component_id] = routeflow2_component

    routeflow3_component = generate.create_container(host1, "rfvm", ContainerType.LXC,
                                                           "ubuntu", BackingStore.LVM, rfvm_scripts)
    components[routeflow3_component.component_id] = routeflow3_component

    routeflow4_component = generate.create_container(host1, "rfvm", ContainerType.LXC,
                                                           "ubuntu", BackingStore.LVM, rfvm_scripts)
    ...
    bridge1_component = generate.create_bridge(host2, BridgeType.OPENVSWITCH, controller, controller_port, switch1dp)
    components[bridge1_component.component_id] = bridge1_component
    br1_id = resources.get_last_id("b")

    bridge2_component = generate.create_bridge(host2, BridgeType.OPENVSWITCH, controller, controller_port, switch2dp)
    components[bridge2_component.component_id] = bridge2_component
    br2_id = resources.get_last_id("b")

    bridge3_component = generate.create_bridge(host2, BridgeType.OPENVSWITCH, controller, controller_port, switch3dp)
    components[bridge3_component.component_id] = bridge3_component
    br3_id = resources.get_last_id("b")

    bridge4_component = generate.create_bridge(host2, BridgeType.OPENVSWITCH, controller, controller_port, switch4dp)
    components[bridge4_component.component_id] = bridge4_component
    br4_id = resources.get_last_id("b")

    client1_component = generate.create_container(host2, "rfc", ContainerType.UNSHARED, scripts=rfclient_scripts)
    components[client1_component.component_id] = client1_component
    client1_id = resources.get_last_id("rfc")

    client2_component = generate.create_container(host2, "rfc", ContainerType.UNSHARED, scripts=rfclient_scripts)
    components[client2_component.component_id] = client2_component
    client2_id = resources.get_last_id("rfc")

    client3_component = generate.create_container(host2, "rfc", ContainerType.UNSHARED, scripts=rfclient_scripts)
    components[client3_component.component_id] = client3_component
    client3_id = resources.get_last_id("rfc")

    client4_component = generate.create_container(host2, "rfc", ContainerType.UNSHARED, scripts=rfclient_scripts)
